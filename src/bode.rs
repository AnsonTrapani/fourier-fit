...
fn fmt_tick_bode(v: f64) -> String {
    if !v.is_finite() { return "NaN".into(); }
...
impl<'a> canvas::Program<Message> for BodeView<'a> {
    ...
    fn draw(
        ...
        let geom = self.cache.draw(renderer, bounds.size(), |frame: &mut Frame| {
            ...
            frame.stroke(
                ...
                Stroke {
                    width: 1.0,
                    style: iced::widget::canvas::Style::Solid(Color { a: 0.22, ..glow_purple() }),
                    ..Stroke::default()
                },
            );
            ...
            let (freqs, mag_db) = match (self.freqs, self.mag_db) {
                ...
                _ => {
                    frame.fill_text(Text {
                        content: "No bode data".into(),
                        position: Point::new((left + right) / 2.0, (top + bottom) / 2.0),
                        color: label_color(),
                        size: 14.0.into(),
                        ..Text::default()
                    });
